...
namespace CDS005.IListWithGenericsDemo
{
    ...
    public class PersonListRepository
    {
        ...
        public static string AverageAge(PersonList persons)
        {
            ...
            foreach(var item in persons)
            {
                totalDays = totalDays + Math.Abs(((now - item.Birthday)).Days);
            }
        ...
        public static List<string> GetFirstName(PersonList persons)
        {
            List<string> fn = new List<string>();//用来装检测出的复姓

            List<string> a = new List<string>(){"欧阳","太史","端木","上官","司马","东方","独孤","南宫","万俟",
...
"东宫","仲长","子书","子桑","即墨","达奚","褚师","吴铭"};//枚举所有复姓
            foreach(var people in persons)//轮训所有人员姓名
            {
                foreach(var item in a)//轮训所有人员姓名
                {
                    if (people.Name.Contains(item.ToString()))
                    {
                        fn.Add(item);
                        continue;
                    }
            ...
            // 复姓人数  fn.count();  非复姓人数 persons.count()-fn.count();
            return fn;
